...
<div class="justify-content-center">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-user fa-sm fa-fw mr-2 text-gray-400"></i>Update Carousell Informasi Baru</h6>
        </div>
        <div class="card-body mx-10">
            <form method="post" action="/dashboard/carousell/{{ $carousel->id }}" class="mb-5" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="mb-3">
                  <label for="title_carousel" class="form-label">Judul Carousell </label>
                  <input type="text" class="form-control @error('title_carousel') is-invalid @enderror" id="title_carousel" name="title_carousel" autofocus required value="{{ old('title_carousel', $carousel->title_carousel) }}">
                ...
                <button type="submit" class="btn btn-primary">Update Carousell Informasi</button>
              </form>
        </div>
    </div>
